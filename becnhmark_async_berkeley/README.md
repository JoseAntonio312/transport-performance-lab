# ASYNC-BERKELEY - Transferencia de ficheros y benchmarking en C++23

Este proyecto implementa una base de trabajo en **C++23** para estudiar transferencia de ficheros sobre **async-berkeley**, con foco en:
...
El objetivo actual del proyecto es disponer de una implementación experimental con **async-berkeley** que sirva como base para estudiar:
...
## Sobre la librería async-berkeley

La librería **async-berkeley** usada en este proyecto **no es de autoría propia**. La autoría de dicha librería corresponde a un tercero, y en este trabajo se utiliza como base de transporte para construir los ejemplos experimentales de:
...
La librería async-berkeley puede mantenerse **fuera de este proyecto**, por ejemplo en un directorio hermano:
...
La concurrencia se gestiona con una estrategia híbrida orientada a comparabilidad experimental:

- **async-berkeley** se usa como capa de sockets;
- **poll()** se mantiene como mecanismo de multiplexación;
- la lógica del servidor conserva la misma estructura general que la versión BSD.
...
La comunicación se gestiona mediante **async-berkeley**, manteniendo una estructura de flujo comparable con la versión BSD sockets.
...
## Adaptación específica al caso async-berkeley

Para poder usar async-berkeley en este proyecto, se realizaron varias adaptaciones respecto a la versión previa con BSD sockets.

### 1. Separación entre librería y proyecto experimental

En lugar de incrustar toda la librería dentro del mismo proyecto, se ha preparado un proyecto independiente de benchmarks que enlaza con la librería async-berkeley ubicada fuera del árbol principal.
...
### 2. Uso de async-berkeley como capa de socket

La implementación basada en async-berkeley sustituye las llamadas BSD directas más representativas por abstracciones de la propia librería, por ejemplo:
...
Esto permite construir servidor, cliente y benchmark conservando una semántica cercana a Berkeley sockets, pero utilizando la interfaz proporcionada por la librería.

### 3. Mantenimiento de `poll()` para comparabilidad experimental

Aunque async-berkeley permite una capa de abstracción superior sobre sockets Berkeley, en este proyecto se ha mantenido explícitamente `poll()` en varios puntos de la implementación.

Esto se hace de forma intencionada para:

- preservar comparabilidad con la versión BSD sockets original;
- mantener una estructura lógica equivalente;
...
## Compilación de la librería async-berkeley

Si la librería async-berkeley no está disponible todavía en el entorno de trabajo, primero hay que disponer de ella en una ruta accesible desde el proyecto experimental.

### Ejemplo de organización fuera del proyecto experimental
...
En este caso, el proyecto experimental puede enlazar con async-berkeley mediante una variable CMake como `ASYNC_BERKELEY_DIR` apuntando al directorio raíz de la librería.

---

## Compilación del proyecto experimental async-berkeley
...
Por tanto, si en el proyecto async-berkeley se conservan:
...
Que una versión basada en async-berkeley obtenga mejores resultados que una versión con BSD sockets en un conjunto concreto de pruebas **no implica automáticamente** que async-berkeley sea siempre más rápido en términos generales.
