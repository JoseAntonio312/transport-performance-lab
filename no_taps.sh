...
ORIG_SUMMARIES_DIR="$GLOBAL_RESULTS_DIR/summaries"
...
echo "[1/3] Copiando summaries existentes excepto TAPS..."

find "$ORIG_SUMMARIES_DIR" -maxdepth 1 -type f -name "*_summary.json" \
  ! -name "taps-asio_summary.json" \
  ! -name "taps_asio_summary.json" \
  ! -name "*taps*summary.json" \
  -exec cp {} "$FILTERED_SUMMARIES_DIR/" \;

echo "[2/3] Generando métricas globales sin TAPS..."
...
echo "[3/3] Mergeando PDFs existentes sin TAPS..."
...
for current, _, files in os.walk(reports_dir):
    if "merged" in os.path.relpath(current, reports_dir).split(os.sep):
        continue
